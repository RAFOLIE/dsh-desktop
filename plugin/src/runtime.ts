...
import { ensureInstalled, ensureUpdated, ensureWebShortcut, exePathOf, nodeDeps, type InstallerDeps } from './installer.js'
import { launchDesktop } from './launcher.js'

// Register this plugin's job kind the designed way: declaration merging into
// the ecosystem's extensible kind map, so `kind: 'desktop'` typechecks and the
// id namespace (`desktop-N`) is reserved for us.
...
export type LaunchValue =
  | { status: 'launched', exePath: string }
  | { status: 'installing', jobId: string }
  | { status: 'windows-only' }
 ...
 */
function startInstallJob(config: ResolvedConfig, deps: RuntimeDeps, logger: Context['logger']): JobHooks {
  ...
  const done = (async (): Promise<JobOutcome> => {
    try {
      ...
      if (abort.signal.aborted) return { status: 'killed', detail: 'cancelled before launch' }
      ...
      if (abort.signal.aborted) return { status: 'killed', detail: 'cancelled during install' }
 ...
 */
export function buildDesktopTool(ctx: Context, config: ResolvedConfig, deps: RuntimeDeps): ToolDefinition {
  const logger = ctx.logger

  return defineTool({
    name: 'desktop_launch',
    description:
      'Launch the DSH desktop app (dsh-desktop-windowos, a Windows tray shell around the webchat). '
    ...
    output: {
      ...
      render: (_args, value) => {
        const v = value as LaunchValue
        if (v.status === 'launched') return [{ type: 'text', text: `DSH desktop app launched: ${v.exePath}` }]
      ...
      presentationMeta: (_args, value) => value as LaunchValue,
    },
    timeoutMs: 300_000,
    presentCall: () => ({ card: 'generic', title: '启动 DSH 桌面端', kind: 'execute' }),
    presentResult: (_args, result) => {
      const meta = result.meta as LaunchValue | undefined
    ...
    async execute(_args, exec): Promise<LaunchValue> {
 ...
 */
export function apply(ctx: Context, config?: Config, deps: RuntimeDeps = runtimeDeps()): void {
  ...
  if (process.platform !== 'win32') {
    ...
    void ensureInstalled(resolved, deps)
      .then(result => {
        logger.info(
          `dsh-desktop-plugin: exe ready at ${result.exePath}`
      ...
      .finally(() => ensureWebShortcut(resolved, deps)
        .then(web => {
          if (web.created) logger.info(`dsh-desktop-plugin: web shortcut ready at ${web.path}`)
        })
        .catch(error => { logger.warn(`dsh-desktop-plugin: web shortcut failed: ${String(error)}`) }))
  }
